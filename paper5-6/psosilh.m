...
data = load('ecolidat.csv'); 
X = data;
k = 8;

CostFunction=@(m) AvgSil(m, X, k);     % Cost Function is a function of m which returns ClusteringCost(m,X)
...
csvwrite('psosil-ecoli-results.csv', finalanswer);
 %csvwrite('psosil-iris-results.csv', finalanswer); 
 %csvwrite('psosil-wine-results.csv', finalanswer);
% csvwrite('psosil-wisc-results.csv', finalanswer);
% csvwrite('psosil-yeast-results.csv', finalanswer);
